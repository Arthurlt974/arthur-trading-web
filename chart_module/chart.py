...
.logo{{font-weight:700;font-size:12px;letter-spacing:0.6px;color:var(--orange);
  padding-right:14px;border-right:1px solid var(--border2);margin-right:14px;white-space:nowrap;}}
.pair{{font-size:15px;font-weight:700;color:var(--text);letter-spacing:0.1px;margin-right:8px;}}
.exch{{font-size:11px;color:var(--faint);letter-spacing:0.3px;margin-right:16px;}}
...
.ohlc-lbl{{font-size:11px;color:var(--faint);letter-spacing:0.3px;text-transform:uppercase;}}
.ohlc-val{{font-size:11px;font-weight:600;}}
.hdr-right{{margin-left:auto;display:flex;align-items:center;gap:12px;}}
.live-badge{{font-size:11px;padding:2px 8px;border-radius:2px;letter-spacing:0.3px;font-weight:700;}}
...
.tf-btn{{padding:3px 9px;border:none;background:transparent;color:var(--muted);
  ...
  border-radius:3px;transition:all .1s;text-transform:uppercase;letter-spacing:0.1px;}}
...
#tooltip .tt-date{{color:var(--muted);font-size:11px;margin-bottom:6px;letter-spacing:0.3px;}}
...
.bstat .lbl{{font-size:11px;color:var(--faint);letter-spacing:0.3px;text-transform:uppercase;}}
...
.qp-title{{
  ...
  letter-spacing:0.6px;color:#FABE2C;
}}
...
.qp-tab{{
  ...
  font-size:11px;letter-spacing:0.3px;color:var(--muted);
...
.tool-name{{
  ...
  font-size:12px;letter-spacing:0.3px;color:var(--text);flex:1;
}}
.tool-card.tc-active .tool-name{{color:#FABE2C;}}
.tool-card.tc-running .tool-name{{color:var(--orange);}}
.tool-tag{{font-size:7px;padding:1px 5px;border-radius:2px;letter-spacing:0.1px;}}
...
.field-lbl{{font-size:11px;color:var(--muted);letter-spacing:0.1px;min-width:72px;}}
.field-inp{{
  flex:1;background:var(--bg);border:1px solid #2A2A2A;
  color:#FF9944;font-family:'IBM Plex Mono', monospace;
...
.run-btn{{
  ...
  color:var(--orange);font-family:'IBM Plex Sans', sans-serif;font-weight:700;
  font-size:11px;letter-spacing:0.6px;cursor:pointer;border-radius:3px;
...
.mode-lbl{{font-size:11px;font-weight:700;letter-spacing:0.3px;text-transform:uppercase;color:var(--text2);}}
...
.mo-text{{flex:1;}}
.mo-lbl{{font-size:11px;font-weight:700;letter-spacing:0.3px;text-transform:uppercase;}}
...
.mo-badge{{font-size:11px;padding:1px 5px;border-radius:2px;letter-spacing:0.1px;
...
<div class="hdr" style="display:{hdr_display}">
  ...
  <div class="live-badge" style="background:rgba(255,152,0,0.08);color:var(--orange);border:1px solid rgba(255,152,0,0.2);font-size:11px;padding:2px 7px;border-radius:2px;letter-spacing:0.3px;">{DATA_SOURCE.upper()}</div>
...
function drawMain() {{
  ...
  for(let s=0;s<=gridSteps;s++) {{
    ...
    // Prix axe droit
    ctx.fillStyle='#444444'; ctx.font='10px IBM Plex Mono,monospace';
    ctx.textAlign='left'; ctx.fillText(fmt(price), W-PAD.r+6, y+4);
  }}

  // ── GRILLE VERTICALE + AXE TEMPS ──
  ctx.fillStyle='#444444'; ctx.font='9px IBM Plex Mono,monospace'; ctx.textAlign='center';
  ...
  if(HOVER_IDX>=0 && HOVER_IDX<N) {{
    ...
    if(HOVER_Y>0) {{
      ...
      ctx.fillStyle='#e8e8e8'; ctx.font='9px IBM Plex Mono,monospace'; ctx.textAlign='left';
    ...
    ctx.fillStyle='#e8e8e8'; ctx.font='9px IBM Plex Mono,monospace';
...
function drawRSI() {{
  ...
  [[70,'rgba(255,34,34,0.5)'],[50,'rgba(100,100,100,0.4)'],[30,'rgba(0,255,65,0.5)']].forEach(([v,col])=>{{
    ...
    ctx.beginPath(); ctx.moveTo(0,y); ctx.lineTo(W-PAD.r,y); ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle=col; ctx.font='8px IBM Plex Mono,monospace';
  ...
  if(last!=null) {{
    ...
    ctx.fillStyle='#000'; ctx.font='bold 8px IBM Plex Mono,monospace'; ctx.textAlign='left';
...
function drawVol() {{
  ...
  // Label
  ctx.fillStyle='#333333'; ctx.font='8px IBM Plex Mono,monospace';
...
function drawSingle(ctx, d, W, H) {{
  ...
  if(d.type === 'line') {{
    ...
    ctx.setLineDash([]);
    ctx.fillStyle=d.color||'#FABE2C'; ctx.font='8px IBM Plex Mono,monospace';
    ...
    if(d.i2==null) {{ ctx.beginPath(); ctx.arc(x1,y1,3,0,Math.PI*2); ctx.fill(); }}
      ...
      levels.forEach((lv, li) => {{
        ...
        ctx.setLineDash([]);
        ctx.fillStyle=cols[li]; ctx.font='8px IBM Plex Mono,monospace';
        ctx.textAlign='left';
    ...
    ctx.fillStyle=d.color||'#FABE2C';
    ctx.font='bold 12px IBM Plex Mono,monospace'; ctx.textAlign='left';
