...
def get_btc_dominance_history():
    try:
        ...
        r = requests.get(url, params={"vs_currency": "usd", "days": "90"}, timeout=10)
        data = r.json()
        ...
        if btc_caps:
            ...
            return pd.DataFrame({"date": dates, "dominance": values})
    except:
        pass
    return pd.DataFrame()
...
def afficher_graphique_pro(symbol, height=600):
    ...
    subplot_titles = (["Prix", "Volume", "RSI", "MACD"] if show_volume
                      else ["Prix", "RSI", "MACD"])
    ...
    for col_name, color, label in [
        ("SMA20", "#00bcd4", "SMA 20"), ("SMA50", "#ff9800", "SMA 50"), ("SMA200", "#e91e63", "SMA 200"),
    ]:
    ...
    if "MACD" in df.columns:
        ...
        fig.add_trace(go.Bar(x=df.index, y=df["MACD_hist"], name="Hist",
                             marker_color=colors_macd, showlegend=False), row=row_offset+2, col=1)
    ...
    fig.update_layout(**PLOTLY_THEME, height=height, xaxis_rangeslider_visible=False,
                      hovermode="x unified",
    ...
    if "RSI" in df.columns and not pd.isna(last["RSI"]):
        ...
        label = "Suracheté" if rsi_val > 70 else "Survendu" if rsi_val < 30 else "Neutre"
        c4.metric("RSI (14)", f"{rsi_val:.1f}", label)
...
def afficher_mini_graphique(symbol, chart_id, period="3mo"):
    ...
    fig.update_layout(**PLOTLY_THEME, height=400, xaxis_rangeslider_visible=False,
                      showlegend=False, title=dict(text=symbol, font=dict(color="#ff9800", size=13)),
...
def afficher_bitcoin_dominance():
    ...
    c1.metric("BTC Dominance", f"{btc_dom:.1f}%")
    c2.metric("ETH Dominance", f"{eth_dom:.1f}%")
    c3.metric("Altcoin Dom.", f"{100 - btc_dom - eth_dom:.1f}%")
    ...
    if not df_dom.empty:
        ...
        fig.add_trace(go.Scatter(x=df_dom["date"], y=df_dom["dominance"], name="BTC.D",
                                 line=dict(color="#ff9800", width=2.5),
                                 fill="tozeroy", fillcolor="rgba(255,152,0,0.08)"), row=1, col=1)
        ...
        fig.update_layout(**PLOTLY_THEME, height=650, hovermode="x unified",
                          title=dict(text="📊 Bitcoin Dominance — CoinGecko",
                                     font=dict(color="#ff9800", size=16)))
        fig.update_yaxes(ticksuffix="%")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Données temporairement indisponibles (limite API). Réessayez dans 1 minute.")
    ...
    if cryptos:
        ...
        fig_heat.update_layout(**PLOTLY_THEME, height=500, margin=dict(l=10, r=10, t=50, b=10),
...
def afficher_graphique_crypto(symbol_input, height=550):
    ...
    if df.empty:
        ...
        if not df_yf.empty:
            df = df_yf.rename(columns={"Open": "open", "High": "high", "Low": "low", "Close": "close"})

    if df.empty:
        st.error(f"Données indisponibles pour {sym}")
        return
    ...
    if chart_type == "Chandeliers":
        ...
        fig.add_trace(go.Candlestick(x=df.index, open=df[open_c], high=df[high_c],
                                     low=df[low_c], close=df[close_col], name=sym,
                                     increasing_line_color="#00ffad", decreasing_line_color="#ff4b4b"), row=1, col=1)
    else:
    ...
    try:
        pr = requests.get(
            ...
            timeout=5).json().get(coin_id, {})
        if pr:
            c1, c2, c3 = st.columns(3)
            c1.metric("Prix Live", f"${pr.get('usd', 0):,.2f}")
            c2.metric("Variation 24h", f"{pr.get('usd_24h_change', 0):+.2f}%")
    ...
    for row_idx in range((len(tickers_list) + 1) // 2):
        ...
        for col_idx in range(cols_per_row):
            ...
            with cols[col_idx]:
                ...
                color_t = "#00ff88" if chg >= 0 else "#ff4b4b"
                fig = go.Figure()
                fig.add_trace(go.Candlestick(
                    x=df.index, open=df["Open"], high=df["High"],
                ...
                fig.add_trace(go.Scatter(x=df.index, y=sma20, name="SMA20",
                                         line=dict(color="#ff9800", width=1.5, dash="dot")))
                fig.update_layout(**PLOTLY_THEME, height=380, xaxis_rangeslider_visible=False,
                                  showlegend=False, margin=dict(l=30, r=10, t=40, b=20),
                                  title=dict(text=f"{ticker}  {last_p:.2f}  ({chg:+.2f}%)",
                                             font=dict(color=color_t, size=13)))
